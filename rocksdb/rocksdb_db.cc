...
// merge operator disabled by default due to link error
        ...
        if (cf_descs.empty())
        {
            s = db_->CreateColumnFamily(rocksdb::ColumnFamilyOptions(opt), "usertable", &cf);
            assert(s.ok());
            cf_descs.push_back(rocksdb::ColumnFamilyDescriptor("usertable", rocksdb::ColumnFamilyOptions(opt)));
        }
                                       ...
                                       std::vector<Field> &values)
    {
        ...
        SerializeRow(values, data);
        rocksdb::WriteOptions wopt;
        rocksdb::Status s = db_->Put(wopt, cf, key, data);
        if (!s.ok())
        {
            throw utils::Exception(std::string("RocksDB Put: ") + s.ToString());
        }
